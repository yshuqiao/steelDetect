...
class AUGFPN(nn.Module):
    ...
    def __init__(self,
                 ...
                 out_channels,
                 num_outs,
                 pool_ratios=[0.1,0.2,0.3],  # notice
                 start_level=0,
                 end_level=-1,
        ...
        # add lateral conv for features generated by ratio-invariant scale adaptive pooling
        ...
        self.high_lateral_conv_attention = nn.Sequential(nn.Conv2d(out_channels*(len(self.adaptive_pool_output_ratio)),out_channels,1),
                                                                   nn.ReLU(),nn.Conv2d(out_channels,len(self.adaptive_pool_output_ratio),3,padding=1))
    ...
    def init_weights(self):
        ...
        for m in self.high_lateral_conv_attention.modules():  # add this
            if isinstance(m, nn.Conv2d):
                xavier_init(m, distribution='uniform')
    ...
    def forward(self, inputs):
        ...
        raw_laterals = [laterals[i].clone() for i in range(len(laterals))]   # we can add this

        # build top-down path

        laterals[-1] += adap_pool_fusion  # add this
        ...
        for i in range(used_backbone_levels - 1, 0, -1):
            ...
            laterals[i - 1] += F.interpolate(
                laterals[i], scale_factor=2, **self.upsample_cfg)
        ...
        # return tuple(outs)
        return tuple(outs),tuple(raw_laterals)  # we can add this,but need to be annotated when test because test doesn't read train_cfg
