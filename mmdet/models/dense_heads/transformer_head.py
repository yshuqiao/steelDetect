...
class TransformerHead(AnchorFreeHead):
    ...
    See pper:End-to-End Object Detection with Transformers
    <https://arxiv.org/pdf/2005.12872>'_ for details.

    Args:
        num_classes(int):Number of categories excluding the background.
        in_channels(int):Number of channels in the input feature map.
        num_fcs(int,optional):Number of fully-connected layers used in 'FFN',
        ...
        loss_cls(dict,optional):Config of the classification .oss.
    ...
    def __init__(self,
        ...
        if class_weight is not None:
            ...
            # relative classification weight of the no_object class.
            bg_cls_weight = loss_cls.get('bg_cls_weight',class_weight)
            assert isinstance(bg_cls_weight,float),'Expected'\
            'bg_cls_weight to have type float.Found'\
            f'{type(bg_cls_weight)}.'
            class_weight=torch.ones(num_classes+1)*class_weight
            # set_background class as the last indice
            class_weight[num_classes]=bg_cls_weight
            loss_cls.update({'class_weight':class_weight})
            if 'bg_cls_eight' in loss_cls:
        ...
        if train_cfg:
            ...
            # DETR ampling=False,so use PseudoSampler
            sampler_cfg = dict(type='PseudoSampler')
            self.sampler = build_sampler(sampler_cfg,context=self)
        self.num_classes = num_classes
        self.cls_out_channels = num_classes + 1  # notice here
    ...
    def forward_sigle(self,x,img_metas):
        """
        Forward function for a single feature level.

        Args:
            x(Tensor):Input feature from backbone's single stage,shape[bs,c,h,w].  # s = batch_size
            img_metas(list[dict]):List of image information.

        Returns:
            all_cls_scores(Tensor):Outputs from classification head,
        ...
        """
        # nstruct binary masks which used for the transformer.
    ...
    @force_fp32(apply_to=('all_cls_score_list','all_bbox_preds_list'))
    def loss(self,
             all_cls_scores_list,
             all_bbox_preds_list,
             gt_bboxes_list,
        ...
        Args:
            all_bbox_preds_list(list[Tensor]):Classification outputs
        ...
        losses_cls,losses_bbox,losses_iou = multi_apply(
            ...
            all_gt_bboxes_list,all_gt_bboxes_ignore_list,img_metas_list,
            all_gt_labels_list
        )
    ...
    def loss_single(self,
        ...
        num_total_pos = torch.clamp(reduce_mean(num_total_pos), min=1).item()  # 将输入input张量每个元素的夹紧到区间 [min,max][min,max]，并返回结果到一个新张量。
    ...
    def get_targets(self,cls_scores_list,
        ...
        num_total_pos = sum((inds.numel() for inds in pos_inds_list))  # numel() 返回数组中元素的个数
    ...
    def _get_bboxes_single(self,
        ...
        det_bboxes[:, 0::2] = det_bboxes[:, 0::2] * img_shape[1]   # 0到结尾，以2为步长依次取值
